...
async function calculateBitcoinPriceChange(exchangeController) {
	...
	let btcPriceObj = await exchangeController.getLatestTick('USDT-BTC'); /// USDT on binance/bittrex or USD for fiat supported exchanges
...
async function getBitcoinPriceChange(exchangeController) {
	...
	let btcPrice = btcPriceObj[0]['C'].toFixed(2);
	...
	logger.debug(`BTC price ${btcPrice}$, 1-min: ${btcPrice1Min}% 5-min: ${btcPrice5Min}% 30-min: ${btcPrice30Min}% 1-hour: ${btcPrice1Hour}% 24-hour: ${btcPrice24Hour}%`);
