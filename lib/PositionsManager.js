...
class PositionsManager {
	...
	constructor() {
		...
		this.positionsManagerBittrexQueue = new Queue('positionsManagerBittrex');

		this.positionsManagerBittrexQueue.process(function(job, done) {

			console.log('positionsManagerBittrex -->');
			// let self = job.data;
		...
		this.positionsManagerBittrexQueue.add(null, {repeat: {cron: '* * * * *'}}); // every minute - check how to do seconds per settings

	}

	_startMonitoringPositions() {

	}

	terminate() {
		positionsManagerBittrexQueue.close();
		this.db.terminate();
	}
