			...
			positions.forEach(async function(position) {
				...
				if (position.exchange === 'bittrex') {
					...
					if (position.simulated) {
						let tick = await self.bittrex.getLatestTick(marketName);
						if (position.status == PositionStatus.OPEN) {
							console.log(`- Analyzing pending position: ${position} with tick ${tick}`);
							if (tick.L <= position.signalPrice) { // && (tick.H > position.signalPrice)) {
								...
								await this.db.updatePositionById(position._id, {
									status: PositionStatus.ACTIVE,
									quantityBought: position.quantity,
									buyPrice: tick.L,
									boughtAt: new Date() //todo: like this?
								}).catch(err => {
									console.log('! monitorPositionsBittex->updatePositionById: '+error);
								});
								console.log(`- Bought simulated position on ${marketName}, qty: ${position.quantity}.`);

							}
						} else if (position.status == PositionStatus.ACTIVE) {
							console.log(`- Analyzing active position: ${position} with tick ${tick}`);
							...
							if (settings.trade.sell_signal_at_profit) {
								...
								if (tick.H >= sellPrice) {
									...
									await this.db.updatePositionById(position._id, {
										...
										closeReason: PositionCloseReason.GAIN,
									...
									console.log(`- Sold simulated position on ${marketName}, qty: ${position.quantityBought}, price: ${sellPrice}, due to gain`);
								}

								if (tick.L <= stopLossPrice) {
									// if lowest value of last candle > needed buy price, assume it was purchased whole
									await this.db.updatePositionById(position._id, {
										status: PositionStatus.CLOSED,
										closeReason: PositionCloseReason.STOPLOSS,
										quantitySold: position.quantityBought,
										sellPrice: stopLossPrice
									}).catch(err => {
										console.log('! monitorPositionsBittex->updatePositionById: ' + error);
									});
									console.log(`- Sold simulated position on ${marketName}, qty: ${position.quantityBought}, price: ${stopLossPrice}, due to stoploss`);
								}

								// do trailing stoploss simulation ...
							} else {
