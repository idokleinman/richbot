				...
				if (position.exchange === 'bittrex') {
					...
					if (position.simulated) {
						let tick = await self.bittrex.getLatestTick(marketName).catch(error => {
							logger.error(`monitorPositionsBittrex -> getLatestTick failed ${error}`);
							return;
						});
						tick = tick[0];

						if (!tick) {
							logger.error('monitorPositionsBittrex -> getLatestTick did not retrieve valid tick data');
							return;
						}

						if (position.status === 'open') {
							logger.debug(`Analyzing open position: ${JSON.stringify(position)} with tick: ${JSON.stringify(tick)}`);
							...
							if (diff > settings.trade.max_time_open_orders) {
								await self.db.updatePositionById(position._id, {
									...
									logger.error('monitorPositionsBittrex -> updatePositionById failed: ' + error);
								});
							...
							if (tick.L <= position.signalPrice) { // && (tick.H > position.signalPrice)) {
								...
								await self.db.updatePositionById(position._id, {
									...
									logger.error('monitorPositionsBittrex -> updatePositionById: '+error);
								});
							...
							logger.debug(`Analyzing active position: ${JSON.stringify(position)} with tick: ${JSON.stringify(tick)}`);
